...
import com.infy.ekart.payment.utility.ErrorInfo;
...
public class PaymentAPI {
	...
	public ResponseEntity<List<CardDTO>> getCardsOfCustomer(@PathVariable("customerEmailId") String customerEmailId,
			@PathVariable("cardType") String cardType) throws EKartPaymentException {
		return ResponseEntity.ok(paymentService.getCustomerCardOfCardType(customerEmailId, cardType));
	}

	@PostMapping(value = "/customer/{customerEmailId}/pay-order")
	@CircuitBreaker(name = "",fallbackMethod = "payForOrderFallback")
	...
	// authenticatePayment() method of PaymentService and assign the return value to
	// transactionDTO
	...
	// PaymentCircuitBreakerService by passing transaction status as
	// TRANSACTION_SUCCESS
	...
	public ResponseEntity<String> payForOrder(
		...
		ResponseEntity<OrderDTO> dtoResponse = template
				.getForEntity("http://CustomerMS/customerorder-api/order/" + orderID, OrderDTO.class);
		transactionDTO.setOrder(dtoResponse.getBody());
		transactionDTO.setTotalPrice(dtoResponse.getBody().getTotalPrice());
		transactionDTO.setTransactionDate(LocalDateTime.now());
		
		TransactionDTO transaction = paymentService.authenticatePayment(customerEmailId, transactionDTO);
		if (transaction.getTransactionStatus() == TransactionStatus.TRANSACTION_SUCCESS) {
			Integer paymentID = paymentService.addTransaction(transaction);
			paymentCircuitBreakerService.updateOrderAfterPayment(transactionDTO.getOrder().getOrderId(),
					transaction.getTransactionStatus().toString());
			return ResponseEntity.ok(environment.getProperty("PaymentAPI.TRANSACTION_SUCCESSFULL_ONE")
					+ transactionDTO.getTotalPrice() + environment.getProperty("PaymentAPI.TRANSACTION_SUCCESSFULL_TWO")
					+ transactionDTO.getOrder().getOrderId()
					+ environment.getProperty("PaymentAPI.TRANSACTION_SUCCESSFULL_THREE")
					+ paymentID);
		} else {
			return ResponseEntity.ok("Transaction Failed Error Happened");
		}
	}
	...
	public ResponseEntity<String> payForOrderFallback(String customerEmailId, TransactionDTO transactionDTO,
		...
		LOGGER.info("Customer with customer Email Id: " + customerEmailId + "\n and Order Id: "
				+ transactionDTO.getOrder().getOrderId());
		return ResponseEntity.badRequest().body(environment.getProperty(exception.getMessage()));
	}
}
